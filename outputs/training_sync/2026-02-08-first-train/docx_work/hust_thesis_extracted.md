...
$`\rightarrow`$ 后端鉴权与权限校验 $`\rightarrow`$ AI
服务选择技能（`mode`）并可选启用 GraphRAG（`_rag` 后缀） $`\rightarrow`$
若需要可验证结果则触发工具调用 $`\rightarrow`$ 回注工具结果与引用证据
$`\rightarrow`$ 生成最终回答并返回 $`\rightarrow`$
...
在既有离线回归指标之外，本文同步了 2026-02-08 首次训练批次结果，用于验证训练与评测流程可复现。该部分结果不替代既有历史回归口径，仅作为阶段性补充。
...
6.  **阶段性训练样本规模**：2026-02-08 同步批次中，首次 all 训练仅 n=5，随机三组回归为每组 n=6 的合成样本，当前结果仅用于链路验证，不可替代真实 `style/tool/rag` 数据上的正式实验结论。
