...
前端包含 Web（React）与移动端（React Native/Expo）两种实现，需分别遵循对应生态的最佳实践，同时保持接口与状态模型的一致性。

#### Web 前端（React）

Web 前端使用 React + TypeScript + Vite（项目目录：`code/frontend-react/`），遵循 React 社区最佳实践，并强调可维护性与可测试性。

**组件设计**。每个组件应当职责单一、边界清晰；复杂业务逻辑优先抽取为自定义 hooks 或 services，避免在页面组件中堆叠实现细节。路由层使用 React Router，状态管理推荐 Zustand（或按模块封装）。
...
移动端基于 React Native（Expo），在遵循 React 社区规范的同时，强调性能与弱网场景的可用性。

**组件与屏幕划分**。屏幕级组件放在 `screens/`，可复用 UI 放在 `components/`；避免在屏幕中混杂大量业务逻辑，必要时拆分为 hooks。

**状态与副作用**。统一使用 React Hooks 管理状态；对异步请求使用 AbortController 处理取消，避免“卸载后更新状态”的错误。

**列表与性能**。长列表使用 `FlatList`/`SectionList`，开启虚拟化与 `keyExtractor`；对耗时渲染的子组件做 `memo` 或 `useMemo`。

**存储与离线**。本地缓存使用 AsyncStorage，必须限定容量并提供清理入口；关键缓存结构需在文档中说明。

**错误与提示**。网络错误与业务错误要区分提示；超时、取消、鉴权失败应有明确文案或引导。
